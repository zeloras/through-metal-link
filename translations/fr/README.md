...
**Statut :** étape 0 — préparation · 💰 **[prime de 250 $ pour la première construction indépendante](https://github.com/zeloras/through-metal-link/issues)** · liste de courses : [QUICKSTART.md](QUICKSTART.md)
...
La documentation est multilingue : l'anglais est la langue principale et se trouve aux chemins canoniques ; toutes les autres langues reproduisent l'arborescence sous [translations/](..). Modifiez n'importe quelle langue — le CI traduit et valide les autres (voir [CONTRIBUTING.md](CONTRIBUTING.md)).
...
|---|---|---|---|
| 1. Carte de balayage | réponse en fréquence du canal "Langevin–3 mm acier–Langevin" | résonance de la paire trouvée, tracé dans [experiments/001](experiments/001-sweep-map-3mm-steel/README.md) | [sim1](../../docs/img/sim1-sweep-contacts.png), [sim2](../../docs/img/sim2-pair-mismatch.png) |
...
| 4. Nœud | ESP32 + capteur dans une boîte soudée, alimenté et télémétré par le son seul | ≥1 h de fonctionnement autonome | [sim4](../../docs/img/sim4-power-budget.png) |
| 5. Publication | le dépôt devient public, article/tutoriel | reproduction par un tiers | — |
...
| Mode | Fréquence | Résonance définie par | Rendement | Statut |
|---|---|---|---|---|
...
Un modèle de canal semi-empirique (pas de FEM, **pas de données de labo** — intuition pour "à quoi devrait ressembler le balayage et sur quoi viser"). Les hypothèses sont explicites dans `channel_sim.py` (Q chargé ≈40, facteurs k de contact, rendement de la chaîne η≤40%). Régénérer avec : `python3 channel_sim.py --out ../../docs/img`.
...
**Pourquoi 4 transducteurs Langevin, et non 2.** Avec un Q≈40, un désaccord de résonance de 1.5 kHz au sein de la paire fait chuter la puissance du modèle d'un facteur ~10× :

<img src="../../docs/img/sim2-pair-mismatch.png" width="720">

**Étape 3 — données.** Le OOK se heurte à la résonance de l'oscillateur (modèle Q~40 → τ≈0.3 ms) : 1 kbit/s est propre, à 5 kbit/s l'œil est fermé. Aller plus vite nécessite le mode B :
...
1. **Des dizaines à des centaines de volts sur le piézo** une fois le pilote de l'étape 2 en ligne — le TVS du côté réception est mis en place AVANT la première exécution sous tension ; ne touchez pas les fils.
2. **Secteur** — uniquement via une alimentation de laboratoire / isolation ; les cartes de pilote de nettoyeur à ultrasons sont liées galvaniquement au secteur.
3. **Oreilles** — à une puissance non négligeable, utilisez les transducteurs pressés contre du métal ; ne faites jamais fonctionner des ultrasons aéroportés de haute puissance sans une enceinte.
