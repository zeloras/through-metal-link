...
Una piattaforma aperta per il trasferimento di potenza e dati ultrasonici attraverso pareti metalliche solide — "attraverso l'acciaio senza un solo foro", realizzata con mezzi alla portata di un garage.

**Provalo ora (nessun hardware necessario):** `python3 software/sweep-map/sweep_map.py --mock`
...
I documenti sono multilingua: l'inglese è la lingua principale e si trova nei percorsi canonici; ogni altra lingua replica l'albero sotto [translations/](..). Modifica qualsiasi lingua — la CI traduce e committa le altre (vedi [CONTRIBUTING.md](CONTRIBUTING.md)).
...
Le onde radio non attraversano il metallo (gabbia di Faraday), e una penetrazione via cavo significa un buco, una tenuta e un punto di guasto. Gli ultrasuoni, d'altra parte, attraversano il metallo senza problemi: un elemento piezo su ciascun lato della parete lo trasforma in un canale per alimentazione e dati. La letteratura di laboratorio ha già dimostrato la fisica a livelli considerevoli (RPI: 50 W + 12 Mbit/s attraverso 63,5 mm di acciaio; NASA JPL: fino a ~kW attraverso 5 mm di titanio) — queste sono prove di esistenza con hardware specializzato, non la BOM da garage di questo repo. I brevetti fondamentali sono scaduti, e non esiste ancora una piattaforma aperta e riproducibile — questo repository ne sta costruendo una, a partire da **alimentazione di classe watt e dati kbit/s attraverso 3–5 mm di acciaio** una volta misurato lo stadio 2.

## Roadmap

| Fase | Deliverable | Criterio di successo | Previsione |
|---|---|---|---|
| 1. Mappa di sweep | risposta in frequenza del canale "Langevin–3 mm acciaio–Langevin" | risonanza di coppia trovata, grafico in [experiments/001](experiments/001-sweep-map-3mm-steel/README.md) | [sim1](../../docs/img/sim1-sweep-contacts.png), [sim2](../../docs/img/sim2-pair-mismatch.png) |
| 2. Watt | potenza nel carico alla risonanza | ≥0.5 W attraverso 3 mm di acciaio, protocollo in [experiments/002](experiments/002-watts-3mm-steel/README.md) | [sim4](../../docs/img/sim4-power-budget.png) |
...
| 5. Pubblicazione | repo reso pubblico, articolo/how-to | riproduzione da parte di terzi | — |
...
**Fatto quando (per stadio):** stadio 1 — il picco dello sweep si riproduce su due esecuzioni entro <200 Hz ([esperimenti/001](experiments/001-sweep-map-3mm-steel/README.md)); stadio 2 — ≥0.5 W su un carico noto attraverso 3 mm di acciaio e un LED acceso dal lato RX ([esperimenti/002](experiments/002-watts-3mm-steel/README.md)).
...
| **B** — dischi | 0.6–1 MHz | risonanza di spessore della parete ([pettine](../../docs/img/sim3-thickness-comb.png)) | centinaia di mW, centinaia di kbit/s | diramazione dopo i primi watt; richiede tracciamento automatico della frequenza |

Le principali perdite: disallineamento di risonanza all'interno della coppia (±1 kHz per trasduttori Langevin economici), qualità del contatto acustico (epossidica > grasso accoppiante + morsetto > pressione a secco), disallineamento, deriva di risonanza con la temperatura. La risposta a tutte queste è la stessa: **una mappa di sweep prima di ogni modifica alla configurazione**.
...
**Perché 4 trasduttori Langevin, non 2.** Con Q≈40, un disallineamento di risonanza di 1.5 kHz all'interno della coppia fa crollare la potenza del modello di ~10×:
...
3. **Orecchie** — a potenze non banali, aziona i trasduttori premuti contro il metallo; non far mai funzionare ultrasuoni ad alta potenza in aria senza un contenitore.
...
docs/img/        grafici previsionali (generati da software/simulator/channel_sim.py)
hardware/        BOM, driver (half-bridge), ricevitore (raddrizzatore/harvester)
firmware/        firmware del nodo (ESP32 — stub fino alla fase 4)
software/        script di misurazione (sweep map della risposta in frequenza) e simulatore del canale
...
1. **Riproducibilità da zero.** Chiunque con un saldatore e ~210$ può riprodurre il risultato usando solo questa repo.
...
3. **Igiene brevettuale.** Costruiamo sullo strato scaduto ([docs/01-prior-art.md](docs/01-prior-art.md)); le decisioni sono registrate in [docs/decisions/](docs/decisions/0001-frequency-mode-choice.md).
4. **Prima la misurazione, poi l'opinione.** Una mappa di sweep prima di qualsiasi conclusione sul canale.

## Licenze e brevetti

Codice — Apache-2.0, hardware — CERN-OHL-W v2, documentazione — CC-BY-4.0; testi completi in [LICENSES/](../../LICENSES). Chiunque può fare fork e sviluppare su questa base, uso commerciale incluso; la protezione brevettuale deriva dalle clausole di concessione e ritorsione nelle licenze, oltre a una strategia di arte precedente. Lo schema completo e il protocollo di pubblicazione difensiva: [LICENSES.md](LICENSES.md); regole di contribuzione: [CONTRIBUTING.md](CONTRIBUTING.md).
