...
| 4. Nodo | ESP32 + sensore in una scatola saldata ermeticamente, alimentato e telemetrato solo tramite suono | ≥1 h di funzionamento autonomo | [sim4](docs/img/sim4-power-budget.png) |
| 5. Pubblicazione | repo diventa pubblico, articolo/how-to | riproduzione da parte di terzi | — |
...
**Fatto quando (per stadio):** stadio 1 — il picco dello sweep si riproduce su due esecuzioni entro <200 Hz ([esperimenti/001](experiments/001-sweep-map-3mm-steel/README.md)); stadio 2 — ≥0.5 W in un carico noto attraverso 3 mm di acciaio e un LED acceso dal lato RX ([esperimenti/002](experiments/002-watts-3mm-steel/README.md)).
...
|---|---|---|---|---|
| **A** — Trasduttori Langevin | 40 kHz | la coppia di trasduttori (parete ≪ λ — una "membrana") | watt, kbit/s | modalità di partenza (stadi 1–4, [ADR-0001](docs/decisions/0001-frequency-mode-choice.md)) |
...
Le principali perdite: disallineamento di risonanza all'interno della coppia (±1 kHz per trasduttori Langevin economici), qualità del contatto acustico (epoxy > accoppiante a grasso + morsetto > pressione a secco), disallineamento, deriva di risonanza con la temperatura. La risposta a tutte è la stessa: **una mappa di sweep prima di ogni modifica al setup**.
...
**Perché 4 trasduttori Langevin, non 2.** Con Q≈40, un disallineamento di risonanza di 1.5 kHz all'interno della coppia riduce la potenza del modello di ~10×:

<img src="docs/img/sim2-pair-mismatch.png" width="720">

**Stadio 3 — dati.** L'OOK si scontra con il ringing del risonatore (modello Q~40 → τ≈0.3 ms): 1 kbit/s è pulito, a 5 kbit/s l'occhio è chiuso. Andare più veloce richiede la modalità B:

<img src="docs/img/sim5-ook-datarate.png" width="720">

**Budget di potenza del ricevitore.** Le bande ombreggiate sono **obiettivi** (modalità A 0.5–5 W se lo stadio 2 va a buon fine; modalità B più basso). I primi carichi realistici sono ESP32 / BLE / LED con duty-cycle; il Wi-Fi è mostrato come marcatore di picco di assorbimento, non come promessa continua:
...
1. **Decine o centinaia di volt sul piezo** una volta che il driver dello stadio 2 è attivo — il TVS sul lato ricevitore va messo PRIMA della prima esecuzione alimentata; tieni le mani lontane dai fili.
...
4. **Calore** — un trasduttore Langevin senza morsetto si surriscalda in pochi minuti a potenza; bloccalo prima di aumentare la corrente (solo un breve bring-up elettrico a bassa corrente — vedi il README del driver).
...
docs/img/        grafici attesi (generati da software/simulator/channel_sim.py)
hardware/        BOM, driver (half-bridge), ricevitore (raddrizzatore/harvester)
firmware/        firmware del nodo (ESP32 — stub fino alla fase 4)
software/        script di misurazione (mappa di sweep della risposta in frequenza) e simulatore del canale
...
2. **Ogni esperimento è un protocollo.** Niente "ha funzionato alla meglio": [experiments/TEMPLATE.md](experiments/TEMPLATE.md) è obbligatorio.
