...
Uma plataforma aberta para transferência de potência e dados ultrassônicos através de paredes de metal sólido — "através do aço sem um único furo", construída com recursos de nível de garagem.

**Experimente agora (sem hardware necessário):** `python3 software/sweep-map/sweep_map.py --mock`
...
A documentação é multilíngue: o inglês é o idioma principal e está nos caminhos canônicos; todos os outros idiomas espelham a árvore sob [translations/](..). Edite em qualquer idioma — a CI traduz e faz commit do restante (veja [CONTRIBUTING.md](CONTRIBUTING.md)).

<p align="center"><img src="docs/img/sim0-rig-sketch.png" alt="Rig do estágio 1: Pi → DDS → meia-ponte → transformador → piezo TX | aço | piezo RX → ponte → ADC → Pi" width="900"></p>

## A ideia em um parágrafo

As ondas de rádio não atravessam metal (gaiola de Faraday), e uma penetração por cabo significa um furo, uma vedação e um ponto de falha. O ultrassom, por outro lado, atravessa o metal sem problemas: um elemento piezoelétrico de cada lado da parede o transforma em um canal para energia e dados. A literatura de laboratório já comprovou a física em níveis sérios (RPI: 50 W + 12 Mbit/s através de 63,5 mm de aço; NASA JPL: até ~kW através de 5 mm de titânio) — essas são provas de existência com hardware especializado, não a BOM de garagem deste repositório. As patentes fundamentais já expiraram, e ainda não existe uma plataforma aberta e reproduzível — este repositório está construindo uma, começando em **energia da classe de watts e dados na faixa de kbit/s através de 3–5 mm de aço** assim que o estágio 2 for medido.
...
| 5. Publicação | repositório torna-se público, artigo/how-to | reprodução por um terceiro | — |
...
**Pronto quando (por estágio):** estágio 1 — o pico do sweep se reproduz em duas execuções com diferença <200 Hz ([experiments/001](experiments/001-sweep-map-3mm-steel/README.md)); estágio 2 — ≥0.5 W em uma carga conhecida através de 3 mm de aço e um LED aceso no lado RX ([experiments/002](experiments/002-watts-3mm-steel/README.md)).
...
As principais perdas: desalinhamento de ressonância dentro do par (±1 kHz para transdutores Langevin baratos), qualidade do contato acústico (epóxi > graxa de acoplamento + braçadeira > pressão a seco), desalinhamento, desvio de ressonância com a temperatura. A resposta para todas elas é a mesma: **um mapa de sweep antes de cada mudança na configuração**.
...
**Estágio 1 — sweep.** Um pico estreito perto de ~40 kHz; os multiplicadores de contato de espaço reservado do modelo são graxa:seco:folga = 1 : 0.25 : 0.02 (ou seja, graxa ≈4× seco e ≈50× folga de ar). Sem pico significa um problema com o contato ou o par:

<img src="docs/img/sim1-sweep-contacts.png" width="720">

**Por que 4 transdutores Langevin, e não 2.** Sob Q≈40, uma diferença de ressonância de 1.5 kHz dentro do par reduz a potência do modelo em ~10×:
...
**Orçamento de potência do receptor.** Bandas sombreadas são **metas** (modo A 0.5–5 W se o estágio 2 der certo; modo B menor). As primeiras cargas realistas são ESP32 / BLE / LED com ciclo de trabalho; o Wi-Fi é mostrado como um marcador de pico de consumo, não uma promessa contínua:
...
1. **Dezenas a centenas de volts no piezo** assim que o driver do estágio 2 estiver online — o TVS no lado de recepção entra ANTES da primeira execução energizada; mantenha as mãos longe dos fios.
2. **Rede elétrica** — apenas através de uma fonte de bancada / isolamento; placas de driver de limpa-ultrassom são ligadas galvanicamente à rede.
...
4. **Calor** — um transdutor Langevin sem braçadeira superaquece em minutos sob potência; fixe antes de aumentar a corrente (apenas um breve bring-up elétrico de baixa corrente — veja o README do driver).
...
hardware/        BOM, driver (half-bridge), receptor (retificador/coletor)
...
2. **Todo experimento é um protocolo.** Nada de "mais ou menos funcionou": [experiments/TEMPLATE.md](experiments/TEMPLATE.md) é obrigatório.
