...
고체 금속 벽을 통한 초음파 전력 및 데이터 전송을 위한 오픈 플랫폼 — "구멍 하나 없이 강철을 통과", 개러지 수준의 수단으로 구축됨.

**지금 바로 체험하기 (하드웨어 불필요):** `python3 software/sweep-map/sweep_map.py --mock`

**진입 경로:**
- **A — 드라이런:** 모의 스윕 + [시뮬레이터](../../software/simulator/channel_sim.py) (벤치 불필요)
- **B — 빌드 단계 1:** [QUICKSTART.md](QUICKSTART.md) → [experiments/001](experiments/001-sweep-map-3mm-steel/README.md)
- **C — 하드웨어 없이 기여하기:** 선행 기술 / 문서 / 번역 / ADR 댓글 ([CONTRIBUTING.md](CONTRIBUTING.md))

**상태:** 단계 0 — 준비 중 · **아직 하드웨어 검증 없음** (시뮬레이터 전용; 첫 빌드에 현상금 지급) · 💰 **[$250 현상금](https://github.com/zeloras/through-metal-link/issues/5)** · 구매 목록: [QUICKSTART.md](QUICKSTART.md)
...
전파는 금속을 통과하지 못합니다(패러데이 새장), 그리고 케이블 관통은 구멍, 실링, 그리고 고장 지점을 의미합니다. 반면 초음파는 금속을 아주 잘 통과합니다: 벽 양쪽에 압전 소자를 하나씩 배치하면 전력과 데이터를 위한 채널이 됩니다. 실험실 문헌에서는 이미 상당한 수준에서 물리학이 증명되었습니다(RPI: 강철 63.5 mm를 통과해 50 W + 12 Mbit/s; NASA JPL: 티타늄 5 mm를 통과해 최대 ~kW) — 이들은 특수 하드웨어를 사용한 존재 증명이지, 이 저장소의 가라지 BOM이 아닙니다. 기초 특허는 만료되었고, 아직 공개되고 재현 가능한 플랫폼은 존재하지 않습니다 — 이 저장소는 그런 플랫폼을 구축하고 있으며, 2단계가 측정되면 **3–5 mm 강철을 통한 와트급 전력과 kbit/s 데이터**에서 시작합니다.

## 로드맵

| 단계 | 산출물 | 성공 기준 | 예상 |
|---|---|---|---|
| 1. 스윕 맵 | "Langevin–3 mm 강철–Langevin" 채널의 주파수 응답 | 쌍 공명 발견, [experiments/001](experiments/001-sweep-map-3mm-steel/README.md)의 플롯 | [sim1](docs/img/sim1-sweep-contacts.png), [sim2](docs/img/sim2-pair-mismatch.png) |
| 2. 와트 | 공명 시 부하로 전달되는 전력 | 3 mm 강철을 통해 ≥0.5 W, [experiments/002](experiments/002-watts-3mm-steel/README.md)의 프로토콜 | [sim4](docs/img/sim4-power-budget.png) |
| 3. 데이터 | 동일한 쌍을 통한 FSK/OOK | ≥1 kbit/s 무오류 | [sim5](docs/img/sim5-ook-datarate.png) |
| 4. 노드 | 밀폐 용접 박스 내 ESP32 + 센서, 소리만으로 전원 공급 및 원격 측정 | ≥1 h 자율 운영 | [sim4](docs/img/sim4-power-budget.png) |
| 5. 출판 | 최초 독립 재현 + 기사/사용법 + Zenodo 스냅샷 | 제3자 재현 문서화 | — |
...
| 모드 | 주파수 | 공진 설정 기준 | 산출물 | 상태 |
|---|---|---|---|---|
| **A** — Langevin 트랜스듀서 | 40 kHz | 트랜스듀서 페어 (벽 ≪ λ — "멤브레인") | 와트, kbit/s | 시작 모드 (1–4단계, [ADR-0001](docs/decisions/0001-frequency-mode-choice.md)) |
| **B** — 디스크 | 0.6–1 MHz | 벽의 두께 공진 ([콤](docs/img/sim3-thickness-comb.png)) | 수백 mW, 수백 kbit/s | 첫 와트 달성 후 분기; 자동 주파수 추적 필요 |
...
1. **제로에서의 재현성.** 인두기와 약 $210만 있으면 누구나 이 저장소만으로 결과를 재현할 수 있다.
2. **모든 실험은 프로토콜이다.** "대충 작동했다"는 금지: [experiments/TEMPLATE.md](experiments/TEMPLATE.md)는 필수다.
3. **특허 위생.** 만료된 계층 위에 구축한다([docs/01-prior-art.md](docs/01-prior-art.md)); 의사결정은 [docs/decisions/](docs/decisions/0001-frequency-mode-choice.md)에 기록된다.
4. **측정이 먼저, 의견은 그 다음.** 채널에 대한 어떤 결론보다 먼저 스윕 맵을 작성한다.

## 라이선스 및 특허

코드 — Apache-2.0, 하드웨어 — CERN-OHL-W v2, 문서 — CC-BY-4.0; 전체 텍스트는 [LICENSES/](../../LICENSES)에 있습니다. 누구나 포크하여 이를 기반으로 구축할 수 있으며, 상업적 이용도 포함됩니다; 특허 보호는 라이선스의 부여 및 보복 조항과 선행 기술 전략을 통해 제공됩니다. 전체 체계 및 방어적 공개 프로토콜: [LICENSES.md](LICENSES.md); 기여 규칙: [CONTRIBUTING.md](CONTRIBUTING.md).
