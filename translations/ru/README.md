...
> [English (primary)](../../README.md) · Русский · [Deutsch](../de/README.md) · [Português](../pt/README.md)
...
Радиоволны не проходят через металл (клетка Фарадея), а кабельный ввод — это отверстие, уплотнение и точка отказа. Ультразвук металл проходит отлично: пьезоэлемент с каждой стороны стенки превращает её в канал для энергии и данных. Lab литература уже доказала физику на серьёзных уровнях (RPI: 50 Вт + 12 Мбит/с через 63,5 мм стали; NASA JPL: до ~кВт через 5 мм титана) — это доказательства существования с помощью специализированного оборудования, не BOM этого репо. Основные патенты истекли, и нет открытой, воспроизводимой платформы — этот репозиторий её создаёт, начиная с **ватт-класса энергии и кбит/с данных через 3–5 мм стали** после измерения этапа 2.
...
|---|---|---|---|---|
| **А** — ланжевены | 40 кГц | пара трансдьюсеров (стенка ≪ λ — «мембрана») | ватты, кбит/с | стартовый (этапы 1–4, [ADR-0001](docs/decisions/0001-vybor-chastotnogo-rezhima.md)) |
...
3. **Уши** — при мощности работать только с прижатыми к металлу излучателями; мощный воздушный УЗ не включать без корпуса.
...
Архитектурные решения фиксируются в [docs/decisions/](docs/decisions/0001-vybor-chastotnogo-rezhima.md) (ADR).
...
3. **Патентная гигиена.** Строим от истёкшего слоя ([docs/01-prior-art.md](docs/01-prior-art.md)); решения фиксируем в [docs/decisions/](docs/decisions/0001-vybor-chastotnogo-rezhima.md).
